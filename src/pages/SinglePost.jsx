import axios from "axios";
import React, { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import { formatISO9075 as ISO } from "date-fns";
import { toast } from "react-toastify";

const Post = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const [post, setPost] = useState(null);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [reConfirmDelete, setReConfirmDelete] = useState(false);

  useEffect(() => {
    const getPost = async () => {
      const res = await axios(`/single_post/${id}`);
      console.log(res);
      setPost(res.data);
    };
    getPost();
  }, []);

  if (!post) return "";

  const handleDelete = async () => {
    const res = await axios.delete(`/post/delete/${id}`);
    console.log(res);
    if (res.status === 224) {
      toast.success("Post deletion success");
      navigate("/");
    }
  };

  return (
    <>
      <div>
        <div>
          <img src={`https://test-blog-app-ddgx.onrender.com/${post.cover}`} alt="" />
        </div>
        <h2 className="font-bold text-4xl">{post.title}</h2>

        <div className="flex items-center gap-2">
          <img
            src={`https://test-blog-app-ddgx.onrender.com/${post.author.avatar}`}
            alt=""
            className="w-10 h-10 rounded-full"
          />
          <span>{post.author.userName}</span>

          <div className="flex gap-5 ml-9">
            <Link
              to={`/post/edit/${post._id}`}
              className="rounded-full p-2 hover:bg-green-500/50 backdrop-blur-sm"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="lightgreen"
                className="w-5 h-5"
              >
                <path d="M5.433 13.917l1.262-3.155A4 4 0 017.58 9.42l6.92-6.918a2.121 2.121 0 013 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 01-.65-.65z" />
                <path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0010 3H4.75A2.75 2.75 0 002 5.75v9.5A2.75 2.75 0 004.75 18h9.5A2.75 2.75 0 0017 15.25V10a.75.75 0 00-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5z" />
              </svg>
            </Link>
            {confirmDelete && (
              <>
                <div className="absolute top-0 left-0 right-0 bottom-0 grid place-items-center">
                  <div className="bg-white/30 backdrop-blur-sm w-[300px] rounded p-5">
                    <div className="mb-7">
                      <h1 className="font-semibold text-xl text-center">
                        {reConfirmDelete ? <>For Real?</> : <>Are you sure?</>}
                      </h1>
                      <p className="text-center">
                        {reConfirmDelete ? (
                          <>Did you think it through?</>
                        ) : (
                          <>you want to delete this post?</>
                        )}
                      </p>
                    </div>
                    <div className="flex justify-center gap-7">
                      <button
                        className="bg-red-400 px-5 py-1 rounded"
                        onClick={() => {
                          reConfirmDelete
                            ? handleDelete()
                            : setReConfirmDelete(true);
                        }}
                      >
                        Delete
                      </button>
                      <button
                        className="bg-green-400 px-5 py-1 rounded"
                        onClick={() => {
                          setConfirmDelete(false);
                          setReConfirmDelete(false);
                        }}
                      >
                        Cancel
                      </button>
                    </div>
                  </div>
                </div>
              </>
            )}

            <button
              onClick={() => setConfirmDelete(true)}
              className="rounded-full p-2 hover:bg-red-500/50 backdrop-blur-sm"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="lightcoral"
                className="w-5 h-5"
              >
                <path
                  fillRule="evenodd"
                  d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
          </div>
        </div>

        <div dangerouslySetInnerHTML={{ __html: post.content }} />
      </div>
    </>
  );
};

export default Post;
